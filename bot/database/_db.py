from aiosqlite import connect, Error, IntegrityError
...
class TrialDatabase(BaseDatabase):
    ...
    async def select_trial_entry(trial_id:int) -> model.TrialEntry|None:
        data = await TrialDatabase._select("SELECT * FROM trial_entry WHERE trial_id=?",(trial_id,))
        if data is not None and len(data) != 0:
            return model.TrialEntry(*data[0])
        return None

    @staticmethod
    async def insert_trial_entry(trial_id:int,message_id:int,channel_id:int) -> bool:
        try:
            await TrialDatabase._insert("INSERT INTO trial_entry VALUES (?,?,?)",(message_id,channel_id,trial_id,))
            return True
        except IntegrityError:
            return False

    @staticmethod
    async def update_trial_entry(trial_id:int,message_id:int,channel_id:int) -> bool:
        try:
            await TrialDatabase._insert("UPDATE trial_entry SET message_id=?,channel_id=? WHERE trial_id=?",(message_id,channel_id,trial_id,))
            return True
        except IntegrityError:
            return False
    ...
    async def delete_trial_entry(trial_id:int)->None:
        await TrialDatabase._insert("DELETE FROM trial_entry WHERE trial_id=?",(trial_id,))
        
    ## trial
    @staticmethod
    async def select_all_trial_enabled_ids() -> Generator[int]:
        data = await TrialDatabase._select("SELECT server_id FROM trial WHERE enabled=TRUE")
        if data is not None and len(data) != 0:
    ...
    async def select_trial_log_channel(server_id:int) -> int:
        data = await TrialDatabase._select("SELECT log_channel_id FROM trial WHERE server_id=?",(server_id,))
    ...
    async def select_trial_by_server_id(server_id:int) -> model.Trial|None:
        data = await TrialDatabase._select("SELECT * FROM trial WHERE server_id=?",(server_id,))
        if data is not None and len(data) != 0:
    ...
    async def select_trial_by_guild_id(guild_id:int) -> model.Trial|None:
        data = await TrialDatabase._select("SELECT * FROM trial WHERE guild_id=?",(guild_id,))
        if data is not None and len(data) != 0:
            return model.Trial(*data[0])
        return None
        
    @staticmethod
    async def insert_trial(server_id:int,log_channel_id:int,notify_channel_id:int,entry_channel_id:int,guild_id:int,name:str,enabled:bool=True) -> bool:
        try:
            await TrialDatabase._insert("INSERT INTO trial VALUES (?,?,?,?,?,?,?)",(server_id,enabled,log_channel_id,notify_channel_id,entry_channel_id,guild_id,name,))
            return True
        except IntegrityError:
            return False

    @staticmethod
    async def update_trial(server_id:int,log_channel_id:int,notify_channel_id:int,entry_channel_id:int,name:str,enabled:bool) -> bool:
        try:
            await TrialDatabase._insert("UPDATE trial SET name=?,enabled=?,log_channel_id=?,notify_channel_id=?,entry_channel_id=? WHERE server_id=?",(name,enabled,log_channel_id,notify_channel_id,entry_channel_id,server_id,))
            return True
        except IntegrityError:
            return False
    ...
    async def select_trial_category(id:int) -> model.TrialCategory|None:
        data = await TrialDatabase._select("SELECT * FROM trial_category WHERE id=?",(id,))
        if data is not None and len(data) != 0:
            return model.TrialCategory(*data[0]) 
        return None

    @staticmethod
    async def select_trial_category_active(trial_id:int) -> Generator[model.TrialCategory]:
        data = await TrialDatabase._select("""SELECT tc.* 
                                                FROM trial_category tc
                                                JOIN trial t ON tc.trial_id = t.server_id
                                                WHERE tc.trial_id=? AND t.enabled=TRUE""",(trial_id,))
        if data is not None and len(data) != 0:
            return (model.TrialCategory(*v) for v in data)
        return ()

    @staticmethod
    async def insert_trial_category(trial_id:int,name:str,allow_parallel:bool) -> bool:
        try:
            return await TrialDatabase._insert("INSERT INTO trial_category (trial_id,name,allow_parallel) VALUES (?,?,?)",(trial_id,name,allow_parallel,))
        except IntegrityError:
            return False
    ...
    async def update_trial_category(trial_category_id:int,name:str) -> bool:
        try:
            await TrialDatabase._insert("UPDATE trial_category SET name=? WHERE id=?",(name,trial_category_id,))
            return True
        except IntegrityError:
            return False
    ...
    async def select_trial_task(trial_category_id:int) -> Generator[model.TrialTask]:
        data = await TrialDatabase._select("SELECT * FROM trial_task WHERE trial_category_id=?",(trial_category_id,))
        if data is not None and len(data) != 0:
    ...
    @staticmethod
    async def insert_trial_task(trial_category_id:int,name:str,cooldown:int,reward:str,points:int,bonus_time:int,bonus:str) -> bool:
        try:
            return await TrialDatabase._insert("INSERT INTO trial_task (trial_category_id,name,cooldown,reward,points,bonus_time,bonus) VALUES (?,?,?,?,?,?,?)",(trial_category_id,name,cooldown,reward,points,bonus_time,bonus,))
        except IntegrityError:
            return False

    @staticmethod
    async def update_trial_task(id:int,name:str,cooldown:int,reward:int,points:int,bonus_time:int,bonus:str) -> bool:
        try:
            await TrialDatabase._insert("UPDATE trial_task SET name=?,cooldown=?,reward=?,points=?,bonus_time=?,bonus=? WHERE id=?",(name,cooldown,reward,points,id,bonus_time,bonus,))
            return True
        except IntegrityError:
            return False
    ...
    @staticmethod
    async def select_trial_task_requisite(trial_task_id:int) -> Generator[model.TrialTaskRequisite]:
        data = await TrialDatabase._select("SELECT * FROM trial_task_requisite WHERE trial_task_id=?",(trial_task_id,))
        if data is not None and len(data) != 0:
            return (model.TrialTaskRequisite(*v) for v in data)
        return ()

    @staticmethod
    async def insert_trial_task_requisite(trial_task_id:int,formula:str,goal:int) -> bool:
        try:
            return await TrialDatabase._insert("INSERT INTO trial_task_requisite (trial_task_id,formula,goal) VALUES (?,?,?)",(trial_task_id,formula,goal,))
        except IntegrityError:
            return False
    ...
    async def update_trial_task_requisite(id:int,formula:str,goal:int) -> bool:
        try:
            await TrialDatabase._insert("UPDATE trial_task_requisite SET formula=?,goal=? WHERE id=?",(formula,goal,id,))
            return True
        except IntegrityError:
            return False
    ...
    async def delete_trial_task_requisite(id:int)->None:
        await TrialDatabase._insert("DELETE FROM trial_task_requisite WHERE id=?",(id,))

    ## trial record
    @staticmethod
    async def select_all_trial_active_record_by_guild_id(guild_id:int) -> Generator[model.TrialRecord]:
        data = await TrialDatabase._select("""SELECT tr.* 
    ...
    @staticmethod
    async def select_all_trial_record_by_guild_id(guild_is:int) -> Generator[model.TrialRecord]:
        data = await TrialDatabase._select("""SELECT tr.* 
                                                FROM trial_record tr
                                                JOIN trial_task tt ON tr.trial_task_id = tt.id
                                                JOIN trial_category tc ON tt.trial_category_id = tc.id
    ...
    async def select_trial_active_record(smmo_id:int) -> Generator[model.TrialRecord]:
        data = await TrialDatabase._select("SELECT * FROM trial_record WHERE smmo_id=? AND status=FALSE AND cancelled=FALSE",(smmo_id,))
        if data is not None and len(data) != 0:
            return (model.TrialRecord(*v) for v in data)
        return ()
    ...
    async def select_trial_last_records(smmo_id:int) -> Generator[model.TrialRecord]:
        ...
        if data is not None and len(data) != 0:
            ...
            return (model.TrialRecord(*v) for i,v in enumerate(data,start=1) if i!=len(data))
        return ()

    @staticmethod
    async def select_trial_last_record_by_category(smmo_id:int,category_id:int) -> model.TrialRecord | None:
        data = await TrialDatabase._select("SELECT * FROM trial_record tr JOIN trial_task tt ON tr.trial_task_id=tt.id WHERE smmo_id=? AND tt.trial_category_id=? AND end_time IS NOT NULL ORDER BY end_time DESC LIMIT 1",(smmo_id,category_id,))
        if data is not None and len(data) != 0:
    ...
    async def select_trial_record(trial_task_id:int) -> Generator[model.TrialRecord]:
        data = await TrialDatabase._select("SELECT * FROM trial_record WHERE trial_task_id=?",(trial_task_id,))
        if data is not None and len(data) != 0:
            return (model.TrialRecord(*v) for v in data)
        return ()

    @staticmethod
    async def insert_trial_record(trial_task_id:int,smmo_id:int,user_id:int,start_time:int,end_time:int,update_time:int,start_npc:int,start_steps:int,start_pvp:int,start_levels:int,end_npc:int,end_steps:int,end_pvp:int,end_levels:int,status:bool=False,cancelled:bool=False) -> bool:
        try:
            await TrialDatabase._insert("INSERT INTO trial_record (trial_task_id,smmo_id,user_id,status,cancelled,start_time,end_time,update_time,start_npc,start_steps,start_pvp,start_levels,current_npc,current_steps,current_pvp,current_levels) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",(trial_task_id,smmo_id,user_id,status,cancelled,start_time,end_time,update_time,start_npc,start_steps,start_pvp,start_levels,end_npc,end_steps,end_pvp,end_levels,))
            return True
        except IntegrityError:
            return False

    @staticmethod
    async def update_trial_record(current_npc:int,current_steps:int,current_pvp:int,current_levels:int,upd_timestamp:int,end_timestamp:int,cancelled:bool,status:bool,id:int) -> bool:
        try:
            await TrialDatabase._insert("UPDATE trial_record SET current_npc=?,current_steps=?,current_pvp=?,current_levels=?,update_time=?,end_time=?,cancelled=?,status=? WHERE id=?",(current_npc,current_steps,current_pvp,current_levels,upd_timestamp,end_timestamp,cancelled,status,id,))
            return True
        except IntegrityError:
            return False
    ...
    async def select_all_trial_user(server_id:int) -> Generator[model.TrialUser]:
        data = await TrialDatabase._select("SELECT * FROM trial_user WHERE trial_id=?",(smmo_id,server_id,))
        if data is not None and len(data) != 0:
            return (model.TrialUser(*v) for v in data)
        return ()

    @staticmethod
    async def select_trial_user(smmo_id:int,server_id:int) -> Generator[model.TrialUser]:
        data = await TrialDatabase._select("SELECT * FROM trial_user WHERE smmo_id=?,trial_id=?",(smmo_id,server_id,))
        if data is not None and len(data) != 0:
            return (model.TrialUser(*v) for v in data)
        return ()
    ...
    async def insert_trial_user(server_id:int,smmo_id:int) -> bool:
        try:
            await TrialDatabase._insert("INSERT INTO trial_user VALUES (?,?,0)",(server_id,smmo_id,))
            return True
        except IntegrityError:
            return False
    ...
    async def update_trial_user(trial_id:int,smmo_id:int,points:int) -> bool:
        try:
            await TrialDatabase._insert("UPDATE trial_user SET points=? WHERE smmo_id=? AND trial_id=?",(points,smmo_id,trial_id,))
            return True
        except IntegrityError:
            return False
...
class Database(BaseDatabase):
    ...
    @staticmethod
    async def insert_cmp_lb(channel_id: int, message_id: int, guild_id: int, timestamp: int,category:str,timeframe:str) -> bool:
        try:
    ...
    @staticmethod
    async def update_cmp_lb(channel_id: int, timestamp: int,old_message_id:int, message_id: int) -> bool:
        try:
            await Database._insert(
    ...
    async def select_market_notice_item(item_id:int) -> model.MarketNoticeItem:
        data = await Database._select("SELECT * FROM market_notice_item WHERE item_id=?",(item_id,))
        if data is not None and len(data) != 0:
            return model.MarketNoticeItem(*data[0])
        return None
    
    @staticmethod
    async def insert_market_notice_item(item_id:int,message_id:int,time:int) -> bool:
        try:
            await Database._insert("INSERT INTO market_notice_item VALUES (?,?,?)",(item_id,message_id,time,))
            return True
        except IntegrityError:
            return False
    ...
    async def delete_market_notice_item(item_id:int,message_id:int)->None:
        await Database._insert("DELETE FROM market_notice_item WHERE item_id=? AND message_id=?",(item_id,message_id,))

    @staticmethod
    async def delete_old_market_notice_item()->None:
        await Database._insert("DELETE FROM market_notice_item WHERE time<=strftime('%s', 'now')")
    ...
    async def insert_market_notice(channel_id:int) -> bool:
        try:
            await Database._insert("INSERT INTO market_notice (channel_id) VALUES (?)",(channel_id,))
            return True
        except IntegrityError:
            return False
    ...
    async def delete_market_notice(channel_id:int)->None:
        await Database._insert("DELETE FROM market_notice WHERE channel_id=?",(channel_id,))
    ...
    @staticmethod
    async def select_market_by_author_timestamp(author_id:int,time:int) -> model.Market:
        data = await Database._select("SELECT * FROM market WHERE author_id=? AND time=?",(author_id,time,))
        if data is not None and len(data) != 0:
    ...
    async def select_market_by_cat(category_id:int) -> Generator[model.Market]:
        data = await Database._select("SELECT * FROM market WHERE category=?",(category_id,))
        if data is not None and len(data) != 0:
            return (model.Market(*v) for v in data)
        return ()
    ...
    async def select_market_by_user(author_id:int) -> Generator[model.Market]:
        data = await Database._select("SELECT * FROM market WHERE author_id=?",(author_id,))
        if data is not None and len(data) != 0:
            return (model.Market(*v) for v in data)
        return ()
    
    @staticmethod
    async def insert_market_item(title:str,description:str,category:str,price:str,author_id:int,author_smmo_id:int,author_name:int,time:int) -> bool:
        try:
            await Database._insert("INSERT INTO market (title,description,category,price,author_id,author_smmo_id,author_name,time) VALUES (?,?,?,?,?,?,?,?)",(title,description,category,price,author_id,author_smmo_id,author_name,time,))
            return True
        except IntegrityError:
            return False
    ...
    async def delete_old_market_item()->None:
        await Database._insert("DELETE FROM market WHERE time<=CAST(strftime('%s', 'now') AS INTEGER)")

    ## join
    @staticmethod
    async def select_join_roles(guild_id:int) -> model.JoinConf|None:
        data = await Database._select("SELECT * FROM join_conf WHERE guild_id=?",(guild_id,))
        if data is not None and len(data) != 0:
            return model.JoinConf(*data[0])
        return None

    @staticmethod
    async def insert_join_roles(guild_id:int, message:str, guildmates_roles:str, visitators_roles:str,channel:int) -> bool:
        try:
            await Database._insert("INSERT INTO join_conf (guild_id,msg,groles,vroles,channel) VALUES (?,?,?,?,?)",(guild_id,message,guildmates_roles,visitators_roles,channel,))
            return True
        except IntegrityError:
            return False
    ...
    @staticmethod
    async def select_last_season_id() -> int | None:
    ...
    async def insert_season(id:int,name:str,starts_at:str,ends_at:str) -> bool:
        try:
            await Database._insert("INSERT INTO season (id,name,starts_at,ends_at) VALUES (?,?,?,?)",(id,name,starts_at,ends_at,))
            return True
        except IntegrityError:
            return False
    ...
    @staticmethod
    async def insert_statistics(id:int,time:int) -> bool:
        try:
            await Database._insert("""
    ...
    async def insert_delmsg(message_id:int,channel_id:int,time:int) -> bool:
        try:
            await Database._insert("INSERT INTO del_msg VALUES(?,?,?)",(message_id,channel_id,time,))
            return True
        except IntegrityError:
            return False
    ...
    async def update_delmsg(message_id:int,time:int) -> bool:
        try:
            await Database._insert("UPDATE del_msg SET time=? WHERE message_id=?",(time,message_id,))
            return True
        except IntegrityError:
            return False
    ...
    async def delete_delmsg(message_id:int,channel_id:int)->None:
        await Database._insert("DELETE from del_msg WHERE message_id=? AND channel_id=?",(message_id,channel_id,))
    ...
    @staticmethod
    async def insert_best(smmo_id:int,name:str,category:str,date:int,levels:int,steps:int,npc:int,pvp:int) -> bool:
        try:
            await Database._insert("INSERT INTO best VALUES(?,?,?,?,?,?,?,?)",(smmo_id,name,category,date,levels,steps,npc,pvp))
            return True
        except IntegrityError:
            return False
    ...
    async def delete_best(smmo_id: int, category: str) -> None:
        await Database._insert("DELETE from best WHERE smmo_id=? AND category=?",(smmo_id,category,))

    @staticmethod
    async def delete_best_bulk(smmo_ids) -> None:
        placeholders = ', '.join(['?'] * len(smmo_ids))
        await Database._insert(f"DELETE FROM best WHERE smmo_id IN ({placeholders})",(*smmo_ids,))
    ...
    async def insert_task(channel_id: int, guild_id: int, role_id: int) -> bool:
        try:
            await Database._insert("INSERT INTO task VALUES(?,?,?)",(channel_id,guild_id,role_id,))
            return True
        except IntegrityError:
            return False
    ...
    @staticmethod
    async def insert_valut(code:int,year:int,month:int,day:int,note:str) -> bool:
        try:
            await Database._insert("INSERT INTO valut VALUES(?,?,?,?,?)",(code,year,month,day,note,))
            return True
        except IntegrityError:
            return False
    ...
    async def delete_valut(year: int, month: int, day: int) -> None:
        await Database._insert("DELETE from valut WHERE year=? AND month=? AND day=?",(year,month,day,))
    ...
    async def insert_valutmsg(channel_id: int, role_id: int,message_id:int) -> bool:
        try:
            await Database._insert("INSERT INTO valutmsg VALUES(?,?,0,?,0)",(channel_id,role_id,message_id,))
            return True
        except IntegrityError:
            return False
    ...
    @staticmethod
    async def update_valutmsg(status: int, channel_id: int, code:str, message_id:int) -> bool:
        try:
            await Database._insert("UPDATE valutmsg SET status=?, code=?, message_id=? WHERE channel_id=?",(status,code,message_id,channel_id,))
            return True
        except IntegrityError:
            return False
    ...
    async def data_for_best(smmo_id:int):
        ...
        data = await Database._select("SELECT user_stats.smmo_id,year,month,day,time,level,steps,npc_kills,user_kills,quest_performed,bounties_completed,reputation,chests_opened FROM user_stats WHERE user_stats.smmo_id=?",(smmo_id,))
        if data is not None and len(data) != 0:
            aa = pd.DataFrame(
                data,
                columns=("smmo_id","year","month","day","time","level","steps","npc_kills","user_kills","quest_performed","bounties_completed","reputation","chest_opened")
                ).astype(
                     {
                        "smmo_id": "uint16",
                        "year": "uint16",
    ...
    @staticmethod
    async def select_user_stat_data(year: int, month: int, day: int) -> Generator[model.UserStat]:
        data = await Database._select("SELECT * FROM user_stats WHERE year=? AND month=? AND day=?",(year,month,day,))
        if data is not None and len(data) != 0:
    ...
    async def select_counter_user_stats(smmo_id:int) -> int:
        data = await Database._select("SELECT count(*) FROM user_stats WHERE smmo_id=?",(smmo_id,))
        if data is not None:
            return int(data[0][0])
        return 0

    @staticmethod
    async def select_user_stat_bulk(smmo_ids: list[int], year: int, month: int, day: int) -> dict:
        if not smmo_ids:
            return {}
        placeholders = ', '.join(['?'] * len(smmo_ids))
        data = await Database._select(f"SELECT * FROM user_stats WHERE smmo_id IN ({placeholders}) AND year=? AND month=? AND day=?",(*smmo_ids,year,month,day,))
        if data is not None and len(data) != 0:
            return {v[0]:model.UserStat(*v) for v in data}
        return {}

    @staticmethod
    async def select_user_stat(smmo_id: int, year: int, month: int, day: int) -> model.UserStat | None:
        data = await Database._select("SELECT * FROM user_stats WHERE smmo_id=? AND year=? AND month=? AND day=?",(smmo_id,year,month,day,))
        if data is not None and len(data) != 0:
    ...
    async def select_all_user_stats(smmo_id:int):
        data = await Database._select("SELECT * FROM user_stats WHERE smmo_id = ?", (smmo_id,))
        if data is not None and len(data) != 0:
            aa = pd.DataFrame(
                data,
                columns=("smmo_id","year","month","day","time","level","steps","npc_kills","user_kills","quest_performed","bounties_completed","reputation","chest_opened")
    ...
    async def insert_user_stat_bulk(data:list) -> bool:
        try:
            await Database._insert_many("INSERT OR IGNORE INTO user_stats VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)",data)
            return True
        except IntegrityError:
            return False
        
    @staticmethod
    async def insert_user_stat(smmo_id:int,year:int,month:int,day:int,time:int,level:int,steps:int,npc_kills:int,user_kills:int,quest_performed:int,bounties_completed:int,reputation:int,chests_opened:int) -> bool:
        try:
            await Database._insert(
    ...
    @staticmethod
    async def update_user_stat(smmo_id:int,year:int,month:int,day:int,quest_performed:int,bounties_completed:int,reputation:int,chests_opened:int) -> bool:
        try:
            await Database._insert("UPDATE user_stats SET quest_performed=?,bounties_completed=?,reputation=?,chests_opened=? WHERE smmo_id=? AND year=? AND month=? AND day=?",
                (
                    quest_performed,
                    bounties_completed,
                    reputation,
    ...
    async def insert_user(discord_id: int, verification: str) -> bool:
        try:
            await Database._insert("INSERT INTO user VALUES (?,NULL,?)", (discord_id, verification))
            return True
        except IntegrityError:
            return False
    ...
    async def update_user(discord_id: int, smmo_id: int) -> bool:
        try:
            await Database._insert("UPDATE user SET smmo_id=?,verification=NULL WHERE discord_id=?",(smmo_id,discord_id,))
            return True
        except IntegrityError:
            return False
    ...
    @staticmethod
    async def select_events_by_starting_day(year:int,month:int,day:int) -> Generator[model.Event]:
        data = await Database._select("SELECT * FROM events WHERE start_year=? AND start_month=? AND start_day=? ORDER BY start_time ASC",(year,month,day,))
        if data is not None and len(data) != 0:
    ...
    async def select_all_global_events(timestamp:int=0) -> Generator[model.Event]:
        data = await Database._select("SELECT * FROM events WHERE end_time>=? AND global_evt=1 ORDER BY start_time ASC",(timestamp,),)
        if data is not None and len(data) != 0:
            return (model.Event(*v) for v in data)
        return ()
    ...
    async def select_all_events(timestamp:int=0) -> Generator[model.Event]:
        data = await Database._select("SELECT * FROM events WHERE end_time>=? ORDER BY start_time ASC",(timestamp,),)
        if data is not None and len(data) != 0:
            return (model.Event(*v) for v in data)
        return ()
    ...
    async def select_all_guild_events(guild_id: int) -> Generator[model.Event]:
        data = await Database._select("SELECT * FROM events WHERE igguild_id=? ORDER BY start_time ASC", (guild_id,))
        if data is not None and len(data) != 0:
            return (model.Event(*v) for v in data)
        return ()
    ...
    async def select_events(guild_id: int, timestamp: int) -> Generator[model.Event]:
        data = await Database._select("SELECT * FROM events WHERE guild_id=? AND end_time>=? ORDER BY start_time ASC",(guild_id,timestamp,))
        if data is not None and len(data) != 0:
            return (model.Event(*v) for v in data)
        return ()
    ...
    @staticmethod
    async def select_events_by_message(guild_id: int, message_id: int) -> model.Event | None:
        data = await Database._select("SELECT * FROM events WHERE guild_id=? AND message_id=?",(guild_id,message_id,))
        if data is not None and len(data) != 0:
            return model.Event(*data[0])
        return None

    @staticmethod
    async def insert_event(start_year:int,start_month:int,start_day:int,start_time:int,guild_id:int,end_year:int,end_month:int,end_day:int,end_time:int,name:str,description:str,event_type:str,guildies_only:int,message_id:int,channel_id:int,team_size:int,custom_image:str,custom_thumbnail:str,global_evt:bool,host:str,igguild_id:int) -> bool:
        try:
            await Database._insert(
    ...
    ## event team
    @staticmethod
    async def select_event_team_all(event_id: int) -> Generator[model.EventPartecipants]:
        data = await Database._select("SELECT et.* FROM event_team et WHERE et.team=? AND et.event_id=?",
            (
                team_id,
                event_id,
            ),
    ...
    @staticmethod
    async def select_event_team(team_id: str, event_id: int) -> Generator[model.EventPartecipants]:
        data = await Database._select("SELECT et.* FROM event_partecipant et WHERE et.team=? AND et.event_id=?",
            (
                team_id,
                event_id,
            ),
    ...
    @staticmethod
    async def insert_event_team(team: int, smmo_id: int, event_id: int, guild_id: int) -> bool:
        try:
            await Database._insert(
    ...
    ## event partecipant
    @staticmethod
    async def select_event_partecipants(event_id: int) -> Generator[model.EventPartecipants]:
        data = await Database._select("SELECT * FROM event_partecipant WHERE event_id = ? ORDER BY team ASC", (event_id,))
        if data is not None and len(data) != 0:
            return (model.EventPartecipants(*v) for v in data)
        return ()

    @staticmethod
    async def select_event_partecipant(event_id: int, smmo_id: int) -> model.EventPartecipants | None:
        data = await Database._select(
    ...
    @staticmethod
    async def select_event_user_partecipants(smmo_id: int) -> Generator[model.EventPartecipants]:
        data = await Database._select(
            "SELECT * FROM event_partecipant WHERE smmo_id = ? ORDER BY event_id DESC", (smmo_id,)
        )
        if data is not None and len(data) != 0:
            return (model.EventPartecipants(*v) for v in data)
        return ()
    ...
    @staticmethod
    async def select_event_user_partecipants_events(smmo_id: int, time: int) -> Generator[model.EventPartecipants]:
        data = await Database._select(
    ...
    async def select_counter_event_user_partecipants(event_id:int) -> int:
        data = await Database._select("SELECT COUNT(smmo_id) FROM event_partecipant WHERE event_id = ?", (event_id,))
        if data is not None and len(data) != 0:
            return int(data[0][0])
        return ()
    ...
    @staticmethod
    async def insert_event_partecipant(smmo_id: int, name: str, discord_id: int, event_id: int, team: str = "") -> bool:
        try:
            await Database._insert(
    ...
    @staticmethod
    async def update_event_partecipant_points(smmo_id: int, event_id: int, points: str) -> bool:
        try:
            await Database._insert(
    ...
    async def insert_event_lb(channel_id: int, message_id: int, event_id: int) -> bool:
        try:
            await Database._insert("INSERT INTO event_lb VALUES(?,?,?)",(channel_id,message_id,event_id,))
            return True
        except IntegrityError:
    ...
    ## guild stats
    @staticmethod
    async def select_guild_stats_data(year: int, month: int, day: int, season: int) -> Generator[model.GuildStats]:
        data = await Database._select(
    ...
    async def select_wb_notification() -> Generator[model.WorldbossNotification]:
        data = await Database._select("SELECT * FROM worldboss_notification ORDER BY seconds_before ASC")
        if data is not None and len(data) != 0:
            return (model.WorldbossNotification(*v) for v in data)
        return ()

    @staticmethod
    async def insert_wb_notification(
        channel_id: int, role_id: int, seconds_before: int, god: bool
    ) -> bool:
        try:
            await Database._insert(
    ...
    @staticmethod
    async def insert_wb_message(channel_id: int) -> bool:
        try:
            await Database._insert(
                "INSERT INTO worldboss_message VALUES(?,0)", (channel_id,)
            )
            return True
        except IntegrityError:
            return False
    ...
    @staticmethod
    async def insert_lb(channel_id: int, message_id: int, guild_id: int, date: str) -> bool:
        try:
            await Database._insert(
    ...
    @staticmethod
    async def update_lb(channel_id: int, date: str, message_id: int) -> bool:
        try:
            await Database._insert(
                "UPDATE leaderboard SET message_id=?,date=? WHERE channel_id=?",
                (
                    message_id,
                    date,
                    channel_id,
    ...
    async def select_staff(guild_id: int) -> Generator[model.Staff]:
        data = await Database._select("SELECT * FROM staff WHERE guild_id=?", (guild_id,))
        if data is not None and len(data) != 0:
    ...
    @staticmethod
    async def insert_orphanage(channel_id:int,message_id:int,role_id:int,tier:int,active:int) -> bool:
        try:
            await Database._insert("INSERT INTO orphanage VALUES(?,?,?,?,?)",(channel_id,role_id,tier,active,message_id,))
            return True
        except IntegrityError:
            return False

    @staticmethod
    async def update_orphanage_active(channel_id: int, tier: int, active: int) -> None:
        try:
            await Database._insert("UPDATE orphanage SET active=? WHERE channel_id=? AND tier=?",(active,channel_id,tier,))
            return True
        except IntegrityError:
            return False
    
    @staticmethod
    async def update_orphanage_msg_id(channel_id: int, tier: int, message_id: int) -> None:
        try:
            await Database._insert("UPDATE orphanage SET message_id=? WHERE channel_id=? AND tier=?",(message_id,channel_id,tier,))
            return True
        except IntegrityError:
            return False
    ...
    async def delete_orphanage(channel_id: int) -> None:
        await Database._insert("DELETE FROM orphanage WHERE channel_id=?",(channel_id,))

    ## requirements
    @staticmethod
    async def select_requirements(guild_id: int) -> model.Requirements | None:
        data = await Database._select("SELECT * FROM requirements WHERE guild_id=?", (guild_id,))
        if data is not None and len(data) != 0:
            return model.Requirements(*data[0])
        return None

    @staticmethod
    async def insert_requirements(guild_id: int, days: int, levels: int, npc: int, pvp: int, steps: int) -> bool:
        try:
            await Database._insert(
    ...
    @staticmethod
    async def insert_diamonds(role_id: int, channel_id: int, min_price: int) -> bool:
        try:
            await Database._insert(
    ...
    async def update_diamonds(last_min_price: str, channle_id: int) -> bool:
        try:
            await Database._insert("UPDATE diamonds SET last_min_price=? WHERE channel_id=?",(last_min_price,channle_id,))
            return True
        except IntegrityError:
            return False
    ...
    async def insert_safe_user(smmo_id: int, guild_id: int) -> bool:
        try:
            await Database._insert("INSERT INTO safe_user VALUES(?,?)",(smmo_id,guild_id,))
            return True
        except IntegrityError:
            return False
    ...
    async def delete_safe_user(smmo_id: int, guild_id: int) -> None:
        await Database._insert("DELETE FROM safe_user WHERE smmo_id=? AND guild_id=?",(smmo_id,guild_id,))

    ## worldboss
    @staticmethod
    async def select_worldboss(timestamp: int) -> Generator[model.WorldBoss]:
        data = await Database._select("SELECT * FROM worldboss WHERE enable_time >= ? ORDER BY enable_time ASC", (timestamp,))
        if data is not None and len(data) != 0:
    ...
    @staticmethod
    async def insert_worldboss(id:int,name:str,avatar:str,level:int,god:int,strength:int,defence:int,dexterity:int,current_hp:int,max_hp:int,enable_time:int,) -> bool:
        try:
            await Database._insert(
    ...
    @staticmethod
    async def select_gains_leaderboard(channel_id: int) -> Generator[model.GainsLeaderboard]:
        data = await Database._select("SELECT * FROM gains_leaderboard WHERE channel_id=?", (channel_id,))
        if data is not None and len(data) != 0:
            return (model.GainsLeaderboard(*v) for v in data)
        return ()

    @staticmethod
    async def insert_gains_leaderboard(channel_id: int, message_id: int) -> bool:
        try:
            await Database._insert("INSERT INTO gains_leaderboard VALUES(?,?)",(channel_id,message_id,))
            return True
        except IntegrityError:
            return False

    @staticmethod
    async def update_gains_leaderboard(channel_id: int, message_id: int) -> bool:
        try:
            await Database._insert("UPDATE gains_leaderboard SET message_id=? WHERE channel_id=?",(message_id,channel_id,))
            return True
        except IntegrityError:
            return False
    ...
    async def delete_gains_leaderboard(channel_id: int) -> None:
        await Database._insert("DELETE FROM gains_leaderboard WHERE channel_id=?", (channel_id,))
    ...
    @staticmethod
    async def insert_raid(channel_id: int, time: int, duration: int, role_id: int) -> bool:
        try:
            await Database._insert("INSERT INTO raid VALUES(?,?,?,?)",(channel_id,time,duration,role_id,))
            return True
        except IntegrityError:
            return False
