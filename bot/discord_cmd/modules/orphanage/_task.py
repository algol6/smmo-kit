...
class OrphanageTask(commands.Cog):
    ...
    async def check_orphanage(self):
        ...
        emb = None
        channels_id = set()
        tf = int((helpers.get_current_date_game() + timedelta(days=1)).timestamp())
        ...
        for tier_data in orphanage_data:
            ...
            for notification_config in orphanage_db:
                if notification_config.tier != int(tier):
                    continue
                if emb is None:
                    emb = helpers.Embed(title="Orphanage Status",
                    ...
                    for i in range(3):
                        ...
                        if not(orphanage_data[i].is_active or orphanage_data[i].has_expired):
                            progression = False
                        ntemp = int((orphanage_data[i].current_value / orphanage_data[i].target_value)*self.BAR_LENGHT)
                        temp = ntemp * ":small_orange_diamond:"
                        temp_status = "Activable :white_check_mark:" if orphanage_data[i].is_active or orphanage_data[i].has_expired else "Inactive :x:"
                        emb.add_field(name=f"Tier {i+1}",
                                    value=self.TEMPLATES[0].format(
                                        bar=f"{temp}{(self.BAR_LENGHT - ntemp) * ":small_blue_diamond:"}",
                                        percentage=orphanage_data[i].percentage,
                                        x=orphanage_data[i].current_value,
                                        y=orphanage_data[i].target_value,status=temp_status) +
                                            (
                                                self.TEMPLATES[1].format(
                                                    bonuses="\n> ".join(orphanage_data[i].effects),
                                                    actived_time=f"<t:{int(datetime.fromisoformat(orphanage_data[i].goal_reached_at).timestamp())}:R>",
                                                    expiring_time1=f"<t:{tf}:R>",
                                                    expiring_time2=f"(<t:{tf}>)"
                                                ) if orphanage_data[i].is_active else ""
                                            ),
                                        inline=False)
                    emb.set_footer(text="Updated every 5 minutes")
                if notification_config.channel_id not in channels_id:
                    ...
                    if notification_config.message_id is None:
                        ...
                        msg = await helpers.get_channel_and_edit(
                            self.client,
                ...
                if notification_config.active != 1 and tier_data.is_active:
                    await helpers.get_channel_and_edit(
                        ...
                        content=f"<@&{notification_config.role_id}>\nOrphanage **tier {tier}** is now activable.",
                        delete_after=tf-int(datetime.now().timestamp())
                    )
