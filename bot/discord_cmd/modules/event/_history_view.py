...
class HistoryView(discord.ui.View):
    ...
    async def send(self, ctx: discord.ApplicationContext):
        ...
        await self.update_message(self.event_list[:self.sep])
    ...
    async def update_buttons(self):
        ...
        if self.current_page == int(len(self.event_list)/self.sep) + 1:
    ...
    async def create_embed(self, data):
        ...
        emb.set_footer(text=f"Page {self.current_page}/{(len(self.event_list)) // self.sep + 1}")
        return emb
    ...
    async def last_button(self, button:discord.ui.Button, interaction:discord.Interaction):
        await interaction.response.defer()
        self.current_page = int((len(self.event_list)) / self.sep) + 1
        await self.update_message(self.get_current_page_data())
