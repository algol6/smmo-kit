...
class ParticipantsView(discord.ui.View):
    ...
    async def send(self, ctx: discord.ApplicationContext):
        ...
        await self.update_message(self.event_participants[:self.sep])
    ...
    async def update_buttons(self):
        ...
        if self.current_page == int(len(self.event_participants)/self.sep) + 1:
    ...
    def get_current_page_data(self):
        ...
        if self.current_page == int((len(self.event_participants)) / self.sep) + 1:
            ...
            until_item = len(self.event_participants)
        return self.event_participants[from_item:until_item]
    ...
    async def last_button(self, button:discord.ui.Button, interaction:discord.Interaction):
        await interaction.response.defer()
        self.current_page = int((len(self.event_participants)) / self.sep) + 1
        await self.update_message(self.get_current_page_data())
