...
class WorldbossTasks(Cog):
    ...
    async def check_worldboss(self):
        if self.wb_cache is None or self.wb_cache.enable_time < datetime.now().timestamp():
            temp = tuple(await Database.select_worldboss(int(datetime.now().timestamp()-65)))
            if len(temp) == 0:
                return
            self.wb_cache = temp[0]
        
        await self.notify_wb(self.wb_cache)
        await self.update_message(self.wb_cache)
    ...
    async def notify_wb(self, wb:WorldBoss):
        ...
        for d in data:
            ...
            if d.god == 1 and not wb.god:
                continue
            await helpers.get_channel_and_edit(self.client,d.channel_id,content=f"<@&{d.role_id}> world boss **{wb.name}** in about <t:{wb.enable_time}:R> ({d.seconds_before/60:,}m before)", delete_after=(wb.enable_time - datetime.now().timestamp() + 60))
